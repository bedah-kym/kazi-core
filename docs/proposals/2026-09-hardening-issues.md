...
- **#134 (network on/off + allowlist)** [Analysis]
...
- **#126 (credential scoping)** [Analysis]: add "the agent loop stays outside the sandbox; only command execution runs inside".
- **#135 (workspace snapshot/rollback)** [Analysis]: state that a snapshotted workspace is what lets in-workspace commands run without a prompt.
- **#127 (injection corpus)**: add cases where the injected text arrives in shell output and in a file inside the workspace.

## New issues
### N1 [v0.6][P2] Approval telemetry per rule
...
### N2 [v0.6][P3] Connector `preview()` (optional dry run)
...
### N3 [v0.6][P3] Untrusted-context taint flag  *(touches core: human PR only)*
...
### N4 [v0.7][W-A2] Workflow capability manifest + `capability_delta`
...
### N5 [v0.7][W-D2] Shadow replay before a suggestion is shown
...
### N6 [v0.6][P0-ext] Scenario packs + provider continuity test
